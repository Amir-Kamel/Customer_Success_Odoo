...
class CustomerSuccess(models.Model):
    ...
    def _compute_health_percentage(self):
        for rec in self:
            done_matrix = rec.matrix_ids.filtered(lambda m: m.is_done)
            rec.health_percentage = sum(done_matrix.mapped('percentage'))
